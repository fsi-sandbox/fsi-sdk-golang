...
func Signup(c relianceHMO.RelianceHMOCredentials, data []byte, overrideOpts ...req.Option) (string, error) {
	...
	option := append([]req.Option{
		req.WithMethod("POST"),
		req.WithPath("relianceHMO/retail/signup"),
...
func Renew(c relianceHMO.RelianceHMOCredentials, data []byte, overrideOpts ...req.Option) (string, error) {
	var responseString string

	option := append([]req.Option{
		req.WithMethod("PUT"),
		req.WithPath("relianceHMO/retail/renew"),
		req.WithHeader("Sandbox-Key", c.SandboxKey),
		req.WithHeader("Content-Type", "application/json"),
		req.WithBody(data),
	}, overrideOpts...)
